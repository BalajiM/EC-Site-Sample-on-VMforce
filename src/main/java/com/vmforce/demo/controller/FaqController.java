...
public class FaqController {
	...
	// save inquiry as case via JPA
	@RequestMapping(value="/isavefaq.html", method=RequestMethod.POST)
	public ModelAndView saveFromiPhone(Case cace, ModelAndView mv, HttpSession session) { 
		sforce.createCase(cace);
		mv.addObject("infos", sforce.getInformationList(session));
		mv.addObject("products", sforce.getProductList(session));
		mv.addObject("setting", sforce.getSetting(session));
		mv.setViewName("ithank");		
		return mv;
	}		
